...
###########################################################################
## Class Fatigue_detecting 873,535
###########################################################################

COVER = '../fatigue_detecting/images/camera.png'

class Fatigue_detecting(wx.Frame):

    def __init__( self, parent, title ):
        wx.Frame.__init__ ( self, parent, id = wx.ID_ANY, title = title, pos = wx.DefaultPosition, size = wx.Size( 1080,535 ), style = wx.DEFAULT_FRAME_STYLE|wx.TAB_TRAVERSAL )
        ...
        self.m_staticText1 = wx.StaticText( sbSizer3.GetStaticBox(), wx.ID_ANY, u"疲劳时间等级:", wx.DefaultPosition, wx.Size( -1,15 ), 0 )
        self.m_staticText1.Wrap( -1 )
        bSizer6.Add( self.m_staticText1, 0, wx.ALL, 5 )
        m_listBox2Choices = [ u"1",u"2",u"3", u"4" ]
        ...
        sbSizer4 = wx.StaticBoxSizer( wx.StaticBox( sbSizer1.GetStaticBox(), wx.ID_ANY, u"离开或者无法识别检测" ), wx.VERTICAL )
        bSizer8 = wx.BoxSizer( wx.HORIZONTAL )
        self.m_checkBox4 = wx.CheckBox( sbSizer4.GetStaticBox(), wx.ID_ANY, u"离开或者无法识别检测", wx.DefaultPosition, wx.Size( -1,15 ), 0 )
        self.m_checkBox4.SetValue(True) 
        bSizer8.Add( self.m_checkBox4, 0, wx.ALL, 5 )
        
        self.m_staticText2 = wx.StaticText( sbSizer4.GetStaticBox(), wx.ID_ANY, u"离开或者无法识别检测时间(秒):", wx.DefaultPosition, wx.Size( -1,15 ), 0 )
        self.m_staticText2.Wrap( -1 )
        bSizer8.Add( self.m_staticText2, 0, wx.ALL, 5 )
        m_listBox21Choices = [ u"1", u"2", u"3", u"4", u"5", u"6" ]
        ...
        # 闪烁阈值（秒）
        self.AR_CONSEC_FRAMES_check = 30
        self.OUT_AR_CONSEC_FRAMES_check = 30
        # 眼睛长宽比
    ...
    def _learning_face(self,event):
        ...
        self.predictor = dlib.shape_predictor("../fatigue_detecting/model/shape_predictor_face_landmarks.dat")
        ...
        # 获取当前时间，我们假设1分钟闭眼和哈气次数为20为正常值，这也是把一些误差放进去
        ...
        while(self.cap.isOpened()):
            ...
            if(len(faces)!=0):
                ...
                for k, d in enumerate(faces):
                    ...
                    if self.yawn_checkBox1.GetValue()== True:
                        ...
                        if mar > self.MAR_THRESH:# 张嘴阈值0.5
                            ...
                            # 疲劳参数唯1，则如果连续都小于阈值，则表示打了一次哈欠
                            if self.mCOUNTER >= self.MOUTH_AR_CONSEC_FRAMES/2:
                                self.mTOTAL += 1
                                #显示
                        ...
                        cv2.putText(im_rd, "MAR: {:.2f}".format(mar), (330, 60),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        cv2.putText(im_rd, "Yawning: {}".format(self.mTOTAL), (470, 60),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,0), 2)
                    else:
                        pass
                    """
                    眨眼
                    ...
                    if self.blink_checkBox2.GetValue()== True:
                        ...
                        # 循环，满足条件的，眨眼次数+1
                        if ear < self.EYE_AR_THRESH:# 眼睛长宽比：0.25
                            self.COUNTER += 1
                            if self.COUNTER >= self.EYE_AR_CONSEC_FRAMES*2: #两秒
                                cv2.putText(im_rd, "Fatigue warning!!!", (100, 200),cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 3)

                        else:
                            cv2.putText(im_rd, "COUNTER: {}".format(self.COUNTER), (150, 30),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2) 
                            #睁眼后，计算闭眼时间参数
                            # 如果连续10（1/3秒）帧数都小于阈值，则表示进行了一次眨眼活动
                            if self.COUNTER >= self.EYE_AR_CONSEC_FRAMES/2:# 阈值：3
                                self.TOTAL += 1
                                self.m_textCtrl3.AppendText(time.strftime('%Y-%m-%d %H:%M ', time.localtime())+u"疲劳眨眼\n")
                            # 重置眼帧计数器
                            self.COUNTER = 0
                        ...
                        cv2.putText(im_rd, "COUNTER: {}".format(self.COUNTER), (150, 30),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2) 
                        cv2.putText(im_rd, "EAR: {:.2f}".format(ear), (340, 30),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        cv2.putText(im_rd, "Blinks: {}".format(self.TOTAL), (470, 30),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,0), 2)
                    else:
                        pass
                    """
                    瞌睡点头
                    ...
                    if self.nod_checkBox7.GetValue()== True:
                        ...
                        if har > self.HAR_THRESH and ear < self.EYE_AR_THRESH:# 点头阈值0.5,这里瞌睡点头必闭眼，可以结合计算使得点头准确率大幅增加；
                            self.hCOUNTER += 1
                            self.eyeTOTAL +=1
                        else:
                            # 如果连续10帧数次都小于阈值，则表示瞌睡点头一次
                            if self.hCOUNTER >= self.NOD_AR_CONSEC_FRAMES/3:# 阈值：1
                                self.hTOTAL += 1
                                self.m_textCtrl3.AppendText(time.strftime('%Y-%m-%d %H:%M ', time.localtime())+u"点头\n")
                            # 重置点头帧计数器
                            self.hCOUNTER = 0
                            self.eyeTOTAL = 0
                        ...
                        # 显示角度结果
                        cv2.putText(im_rd, "X: " + "{:5.2f}".format(euler_angle[0, 0]), (10, 90), cv2.FONT_HERSHEY_SIMPLEX,0.75, (0, 255, 0), thickness=2)# GREEN
                        cv2.putText(im_rd, "Y: " + "{:5.2f}".format(euler_angle[1, 0]), (180, 90), cv2.FONT_HERSHEY_SIMPLEX,0.75, (255, 0, 0), thickness=2)# BLUE
                        cv2.putText(im_rd, "Z: " + "{:5.2f}".format(euler_angle[2, 0]), (300, 90), cv2.FONT_HERSHEY_SIMPLEX,0.75, (0, 0, 255), thickness=2)# RED    
                ...
                self.oCOUNTER = 0 #检测到人脸就置0
            else:
                # 没有检测到人脸
                self.oCOUNTER+=1
                cv2.putText(im_rd, "No Face", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255),3, cv2.LINE_AA)
                if self.oCOUNTER >= self.OUT_AR_CONSEC_FRAMES_check:    #1秒识别不到人脸
                    self.m_textCtrl3.AppendText(time.strftime('%Y-%m-%d %H:%M ', time.localtime())+u"未识别到人脸，疑似进入睡眠\n")
                    self.oCOUNTER = 0
            ...
            cv2.putText(im_rd, "countdown: {:.2f}".format(a), (10, 120),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,0), 2)
            ...
            if time_difference > 120:
                ...
                self.hTOTAL = 0
                flog_time = datetime.datetime.now()
            # 确定疲劳提示:两分钟疲劳眨眼4次，打哈欠2次，瞌睡点头2次
            if self.TOTAL >= 4 or self.mTOTAL>=2 or self.hTOTAL>=4:
                cv2.putText(im_rd, "Fatigue warning!!!", (100, 200),cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 3)
    ...
    def AR_CONSEC_FRAMES( self, event ):
        ...
        self.AR_CONSEC_FRAMES_check = int(event.GetString())*30
        
    def OUT_AR_CONSEC_FRAMES( self, event ):
        self.m_textCtrl3.AppendText(u"设置未识别人脸时间为:\t"+event.GetString()+"秒\n")
        self.OUT_AR_CONSEC_FRAMES_check = int(event.GetString())*30
